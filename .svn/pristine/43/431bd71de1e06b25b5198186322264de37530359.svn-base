...
<form class="layui-form" lay-filter="form">
    ...
    <div class="layui-content">
        <div class="layui-fluid">
            <a  class="layui-btn layui-btn-primary layui-btn-lg layui-bg-black" href="javascript:history.go(-1);" style="margin-bottom: 1rem;">返回</a>
                    ...
                    <div class="layui-form-item layui-row depot_select">
                        <div class="layui-col-md12 content on">
                            @foreach($data['depot'] as $val)
                                <input type="radio" name="depot" value="{{$val['name']}}" title="{{$val['name']}}" {{$val['name'] == $data['order']['depot'] ? 'checked' : ''}}>
                            @endforeach
                        </div>
                    ...
                    <div class="layui-form-item layui-row">
                        <div class="layui-col-md12 content">
                            @foreach($data['lineData'] as $val)
                                <input type="radio" name="line_id" value="{{$val['id']}}" title="{{$val['name']}}" lay-filter="mimenu" iupid="{{$val['iupid']}}" excel="{{$val['onlineExcel']}}" {{$val['id'] == $data['order']['line_id'] ? 'checked' : ''}} {{$data['order']['status'] >= 4 ? 'disabled' : ''}}>
                            @endforeach
                        </div>
                    </div>
    ...
    layui.config({
        ...
        form.on('submit(layuiadmin-app-form-submit)', function(data){
            ...
            $.ajax({
                ...
                @else
                url : "{{url('user/online/payedStore')}}",
                @endif
                data: formData,
                contentType: false,
                processData: false,
...
<script src="{{asset('js/vue/orderindex.js?t='.time())}}"></script>
<script>
    ...
    @endif
    changeline($("input[name='line_id']:checked"),'',@php echo htmlspecialchars_decode($data['changelineJson']); @endphp);
</script>
</body>
</html>